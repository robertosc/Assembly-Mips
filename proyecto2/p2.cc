...
//Actualizo la cache SRRIP
		if(isHit(tagsSRRIP, index, tag, assoc, &way)==false)
		{
			...
			//std::cout << tagsSRRIP[(index*assoc)+way].replacement;
			missesSRRIP+=1;
		}
		else {
			//tagsSRRIP[(index*assoc)+way].replacement = 0; 
...
int getVictimSRRIP(struct cacheBlock tags[], int index, int assoc, int numSets){
	...
	//Looking with the mem data with .replacement value equal to max, meaning least used
	int max_RRVP = 0;
	int max_idx = 0;

	while(max_RRVP != assoc-1){
		for(int i = 0; i < assoc; i++){
			if(max_RRVP == assoc-1) {
				//std::cout << max_idx << "\n";	
				return max_idx;
			}
			else if (tags[(index*assoc)+i].replacement > max_RRVP){
				max_idx = i;
				max_RRVP = tags[(index*assoc)+i].replacement;
			} 
		}
		if(max_RRVP != assoc-1){
			for(int i = 0; i<assoc; i++){
