...
void updateSRRIP(struct cacheBlock tags[], int index, int way, int assoc, int numTags);
int getVictimSRRIP(struct cacheBlock tags[], int index, int assoc, int numTags);
...
//		setsLRU[i]=0x00010203;
		for(int j=0;j<assoc;j++){
			...
			tagsSRRIP[(i*assoc)+j].replacement=assoc-1;
...
//Actualizo la cache SRRIP
		...
		{
			way = getVictimSRRIP(tagsSRRIP, index, assoc, numSets);
			tagsSRRIP[(index*assoc)+way].tag=tag;
			tagsSRRIP[(index*assoc)+way].valid=true;
			tagsSRRIP[(index*assoc)+way].replacement -= 1;
			missesSRRIP+=1;
		}
		else {
			found = true;	
			updateSRRIP(tagsSRRIP, index, way, assoc, numSets);		
		}	
		//std::cout << "tag: " << tag << "\n";
		if(index == 0){
			if (found) {std::cout << "\nIter" << n << "\nHit\n";}
			else {std::cout << "\nIter" << n <<"\nMiss\n";}
			std::cout << "Insertado en way:" << way << "\nDato con tag:" << tag << "\n";
			std::cout << "Tag      Replacement value\n";
			for(int i = 0; i<assoc; i++){
				std::cout << tagsSRRIP[(index*assoc)+i].tag << "  " << tagsSRRIP[(index*assoc)+i].replacement << "\n";
			}
		}
		n++;
		//break;
...
//SRRIP
void updateSRRIP(struct cacheBlock tags[], int index, int way, int assoc, int numSets){
	tags[(index*assoc)+way].replacement=0;
}

int getVictimSRRIP(struct cacheBlock tags[], int index, int assoc, int numSets){
	//check empty way
	int i;
	for(i=0; i<assoc;i++)
	{
	...
	//look for the victim way
	while(true){
		//Recorremos la caché en búsqueda del primer dato igual a assoc-1
		for(int i = 0; i < assoc; i++){
			// Buscamos el primer tag con val assoc-1
			if (tags[(index*assoc)+i].replacement == assoc-1){
				return i;
			} 
		}

		// Si no se encontró el dato, sumamos a todos
		for(int i = 0; i<assoc; i++){
			if(tags[(index*assoc)+i].replacement < assoc){
				tags[(index*assoc)+i].replacement += 1;
			}
		}
	}
